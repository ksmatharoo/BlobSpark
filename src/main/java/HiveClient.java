...
public class HiveClient {
    ...
    public Table createDummyTable(
        ...
        list.add(new FieldSchema("col1","string","no comment"));
        sd.setCols(list);
    ...
    private String getParquetType(StructField field) {
        if (field.dataType() == DataTypes.StringType) {
            ...
            return "boolean";
        }  else if (field.dataType() == DataTypes.BinaryType) {
            return "binary";
    ...
    public void prepareTable(String dbName, String tableName,
        ...
        if (!metaStoreClient.tableExists(dbName, tableName)) {
            Table partitionedTable = createPartitionedTable(dbName, tableName,
                    new URI(path),
                    columns,
                    partitionKey,
            ...
            metaStoreClient.createTable(partitionedTable);
        } else {
            ...
            metaStoreClient.alter_table(dbName,tableName,partitionedTable);
